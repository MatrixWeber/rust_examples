...
use crate::counters::{Count, LimitCounter, Show};
use defmt::*;
 ...
 */
pub async fn blinking_loop<T: Count + Show>(leds: &mut [Output<'_, AnyPin>; 8], limit_counter: &mut T) {
...
pub async fn blink_led(led: &mut Output<'_, AnyPin>, duration_ms: u64) {
    led.set_high();
